        ...
        for (let i=0;i<lenght;i++) {
            ...
            if (lenght > 5) {
                if (i===0 || i===(lenght-1) || i=== currentPage) {
                    list.push(<li key={i} className={currentPage === i? "active": ""} onClick={this.jumpToNumber.bind(this, i)}>{i+1}</li>);
                    flag = true;
  ...
  render() {
    ...
    return (
        ...
        <ul className="pager-container">
            {
                (pager.currentPage!==0 && Math.ceil(pager.totalCount/pager.pageSize) > 1) ? (<li  className="upstairs" onClick={this.jumpToBefor}>上一页</li>) : null
            }
            {
                this.renderPager()
            }
            {
                pager.currentPage < Math.floor(pager.totalCount/pager.pageSize) ? (<li  className="upstairs" onClick={this.jumpToNext}>下一页</li>) : null
            }
        </ul>
    </div>
    );
